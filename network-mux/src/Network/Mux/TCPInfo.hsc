...
instance Storable StructTCPInfo where
    ...
    peek p = do
      ...
      let tcpi_sndrcv_wscale = 0
          tcpi_delivery_rate_app_limited_fastopen_client_fail = 0 -- XXX can't cope with bitfields
    ...
    poke p StructTCPInfo{..} = do
      (#poke struct tcp_info, tcpi_state)  p tcpi_state
      (#poke struct tcp_info, tcpi_ca_state) p tcpi_ca_state
