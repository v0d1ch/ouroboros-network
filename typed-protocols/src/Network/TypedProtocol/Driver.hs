...
data Driver ps (pr :: PeerRole) bytes failure dstate m =
        ...
        , -- | 'tryRecvMessage' is used to interpret @'Collect' _ (Just k') k@.
          ...
          -- there are cases where a seprate implementation would be simpler or
          -- more performant as it does not need to relay on STM but instead
...
runPeerWithDriver Driver{sendMessage, recvMessage, tryRecvMessage, recvMessageSTM} =
  ...
  where
    ...
    go q@(SingCons q') !dstate (CollectSTM refl k' k) = do
      ...
      -- reason, using a simpler 'CollectSTM' which takes
      -- `STM m Message` as an argumen and computes the result by itself is not
      -- possible.
      r <- atomically $
             Left  <$> stm
         <|> Right <$> k'
    ...
    setDState :: forall (st' :: ps).
                 dstate
              -> DriverState ps pr st' bytes failure dstate m
              -> DriverState ps pr st' bytes failure dstate m
    setDState dstate (DecoderState step  _) = DecoderState   step dstate
    setDState dstate (DriverState        _) = DriverState         dstate
    setDState dstate (DriverStateSTM stm _) = DriverStateSTM stm  dstate
