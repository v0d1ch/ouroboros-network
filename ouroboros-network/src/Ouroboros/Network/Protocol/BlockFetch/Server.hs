...
blockFetchServerPeer
  :: forall block point m a.
     Functor m
  => BlockFetchServer block point m a
  -> Server (BlockFetch block point) NonPipelined Empty BFIdle m a
blockFetchServerPeer (BlockFetchServer requestHandler result) =
 ...
 where
  sendBatch
    :: BlockFetchBlockSender block point m a
    -> Server (BlockFetch block point) NonPipelined Empty BFBusy m a
  ...
  sendBlocks
    :: BlockFetchSendBlocks block point m a
    -> Server (BlockFetch block point) NonPipelined Empty BFStreaming m a
