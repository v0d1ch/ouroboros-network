...
blockFetchClientPeer
  :: forall block point m a.
     Monad m
  => BlockFetchClient block point m a
  -> Client (BlockFetch block point) NonPipelined Empty BFIdle m a
blockFetchClientPeer (BlockFetchClient mclient) =
 ...
 where
  ...
  blockFetchResponsePeer
    :: BlockFetchClient block point m a
    -> BlockFetchResponse block m a
    -> Client (BlockFetch block point) NonPipelined Empty BFBusy m a
  ...
  blockReceiver
    ...
    -> Client (BlockFetch block point) NonPipelined Empty BFStreaming m a
...
data BlockFetchClientPipelined block point m a where
   ...
   BlockFetchClientPipelined
     :: m (BlockFetchIdle       block point (Empty :: Queue (BlockFetch block point)) m a)
...
--
data BlockFetchIdle block point q m a where
...
blockFetchClientPeerPipelined
  :: forall block point m a.
     Monad m
  => BlockFetchClientPipelined block point m a
  -> Client (BlockFetch block point) 'Pipelined Empty BFIdle m a
...
blockFetchClientPeerSender
  :: forall block point (q :: Queue (BlockFetch block point)) m a.
     Monad m
  => BlockFetchIdle block point q m a
  -> Client (BlockFetch block point) 'Pipelined q BFIdle m a
